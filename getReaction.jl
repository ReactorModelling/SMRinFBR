function [rrx, deltaH] = getReaction( T, ...
                                      y, ...
                                      P)
...
for i=1:Nz
    ...
    rrx[1,i]= Krx[1]/(pComp[i,4])^2.5*(pComp[i,1]*pComp[i,5]
        - (pComp[i,4])^3*pComp[i,2]/Keq[1])/(denom[i])^2;
    
    rrx[2,i]= Krx[2]/(pComp[i,4])*(pComp[i,2]*pComp[i,5]
        - (pComp[i,4])*pComp[i,3]/Keq[2])/(denom[i])^2;
    
    rrx[3,i]= Krx[3]/(pComp[i,4])^3.5*(pComp[i,1]*(pComp[i,5])^2
